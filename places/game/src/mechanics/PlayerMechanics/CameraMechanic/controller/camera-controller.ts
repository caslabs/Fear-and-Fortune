...
const camShakeChase = new CameraShaker(
	Enum.RenderPriority.Camera.Value,
	(shakeCFrame) => (camera.CFrame = camera.CFrame.mul(shakeCFrame)),
);

const camShakeGun = new CameraShaker(
	Enum.RenderPriority.Camera.Value,
	(shakeCFrame) => (camera.CFrame = camera.CFrame.mul(shakeCFrame)),
);
...
export default class CameraMechanic implements OnStart, OnInit {
	...
	public onStart(): void {
		...
		Signals.switchToPreviousPlayer.Connect(() => {
			this.switchToPreviousPlayer();
		});

		Signals.switchToNextPlayer.Connect(() => {
			this.switchToNextPlayer();
		});

		//Camera Shaking
		const startChasingShakeEvent = Remotes.Client.Get("StartChasingShake");
		const stopChasingShakeEvent = Remotes.Client.Get("StopChasingShake");
		const playGunShakeEvent = Remotes.Client.Get("PlayGunShake");

		playGunShakeEvent.Connect(() => {
			camShakeGun.Start();
		...
		startChasingShakeEvent.Connect(() => {
			camShakeChase.Start();
			camShakeChase.ShakeSustain(CameraShaker.Presets.Explosion);
			print("Shaking");
		});

		stopChasingShakeEvent.Connect(() => {
			camShakeChase.Stop();
	...
	public enableFirstPerson(): void {
		...
		for (const part of character.GetChildren()) {
			this.antiTrans(part as BasePart);
		}

		print("[INFO] First person enabled");
	}

	private antiTrans(part?: BasePart): void {
		const armParts = [
			"LeftUpperArm",
			"LeftLowerArm",
			"LeftHand",
		...
		print("Parsing " + part?.Name);
		print("Checking if " + part?.Name + " is in " + armParts);
		if (part && armParts.includes(part.Name)) {
			print("TEST");
		}
		if (part && armParts.includes(part.Name)) {
			print("Transparency set to " + part.Transparency);
	...
	public enableFirstPerson2(): void {
		const character = this.characterController.getCurrentCharacter();
		const humanoid = character?.FindFirstChild("Humanoid") as Humanoid;
	...
	public disableHeadBobbing(): void {
		this.headBobbingEnabled = false;
