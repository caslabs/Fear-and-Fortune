...
export class AIJumpscareManager {
	private static instance: AIJumpscareManager;
	...
	public static getInstance(playerStateController: PlayerStateController): AIJumpscareManager {
		if (!this.instance) {
			this.instance = new AIJumpscareManager(playerStateController);
		}
				...
				if (currentState === AIState.TriggerJumpscare) {
					...
					if (character) {
						...
						if (humanoidRootPart && humanoidRootPart.IsA("BasePart")) {
							const lookAt = humanoidRootPart.Position;
							const primaryPart = agent.PrimaryPart;
							if (primaryPart && primaryPart.IsA("BasePart")) {
								print("[MODEL] BEFORE", primaryPart.CFrame);
								const distance = lookAt.sub(primaryPart.Position).Unit;
								...
								*/
								//Stares DIRECTLY
								primaryPart.CFrame = new CFrame(primaryPart.Position, lookAt);
